...
export async function getChatFunctions(onToolEvent) {
  ...
  if (containerAvailable) {
    functions.run_code = defineChatSessionFunction({
      description: 'Run Python code in a sandboxed container and return the output. The container has no network access. The output is automatically shown to the user.',
      params: {
        type: 'object',
        properties: {
          code: {
      ...
      async handler({ code }) {
        if (onToolEvent) {
          onToolEvent({ type: 'toolCall', name: 'run_code', language: 'python', code });
        }
